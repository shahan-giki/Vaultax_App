...
# --- 6. HELPER FUNCTIONS ---
@st.cache_data(ttl=60)  # Cache for 1 minute for better responsiveness
...
with st.sidebar:
    ...
    period = st.selectbox("TIMEFRAME", ["1h", "1d", "5d", "1mo", "3mo", "6mo", "1y", "5y"])
...
# Map period to interval for better granularity
interval_map = {
    "1h": ("1d", "5m"),   # 1 hour of 5-minute data
    "1d": ("1d", "5m"),   # 1 day of 5-minute data
    "5d": ("5d", "15m"),  # 5 days of 15-minute data
    "1mo": ("1mo", "1h"), # 1 month of hourly data
    "3mo": ("3mo", "1d"), # 3 months of daily data
    "6mo": ("6mo", "1d"), # 6 months of daily data
    "1y": ("1y", "1d"),   # 1 year of daily data
    "5y": ("5y", "1wk")   # 5 years of weekly data
}
...
try:
    with st.spinner(f"📡 Loading {ticker} data..."):
        ...
        data_period, data_interval = interval_map.get(period, ("1d", "5m"))
        ...
        if period == "1h":
            ...
            if not hist.empty:
                # Get last 12 periods (1 hour of 5-min data)
                hist = hist.tail(12)
        else:
...
with col_head1:
    ...
    _, interval = interval_map.get(period, ("1d", "5m"))
    st.caption(f"Real-Time Market Data • {period.upper()} • {interval} intervals")
...
with tab1:
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            title='Time',
            tickformat='%H:%M' if period in ["1h", "1d"] else '%b %d' if period in ["5d", "1mo"] else '%Y-%m'
        )
    )
